...
class Craftman(AbstractUser):
    ...
    groups = models.ManyToManyField(Group, related_name="craftman_users", blank=True)
    user_permissions = models.ManyToManyField(Permission, related_name="craftman_users", blank=True)
    ...
    def assign_role_permissions(self):
        ...
        # Create a group name based on the user's username or ID
        group_name = f"user_{self.username}_permissions" if self.username else f"user_{self.id}_permissions"
        
        # Get or create the user's personal permission group
        user_group, created = Group.objects.get_or_create(name=group_name)

        # Clear existing permissions
        user_group.permissions.clear()

        # Map model fields to permission codenames
        permission_mapping = {
            'view_only': 'view',
            'copy': 'copy',
        ...
        # Add permissions based on the user's permission fields
        for field, codename in permission_mapping.items():
            if getattr(self, field, False):
                # Try to get the permission - you might need to adjust the app_label
                permission = Permission.objects.filter(codename=codename).first()
                if permission:
                    user_group.permissions.add(permission)

        # Ensure the user is added to their personal permission group
        if not self.groups.filter(name=group_name).exists():
            self.groups.add(user_group)

        logger.info(f"Permissions assigned to group '{group_name}' for user '{self.username}'")    
...
models.signals.pre_save.connect(fetch_location_pre_save, sender=Craftman)


def save(self, *args, **kwargs):
        if Craftman.objects.filter(user_code=self.user_code).exists():
