...
class KeyUserSerializer(serializers.ModelSerializer):
    ...
    def generate_user_code(self):
        """
        Generate user_code with KU prefix for KeyUsers.
        """
        prefix = "KU"  # Fixed prefix for KeyUsers
        last_user = KeyUsers.objects.filter(user_code__startswith=prefix + "-").order_by('-user_code').first()
    ...
    def create(self, validated_data):
        """
        Create KeyUsers user with auto-generated user_code.
        """
        # Generate user code with KU prefix
